#!/usr/bin/env bash
# OneEmu core build: Genesis Plus GX (메가드라이브 / 마스터 시스템 / 게임 기어 / SG-1000) -> arm64-v8a
#
# Usage (from project root):  bash cores/fceumm/build.sh [--clean]
# Works on macOS (arm64/x86_64) and Ubuntu (GitHub Actions).
set -euo pipefail

CORE_ID="genesisplusgx"
REPO_URL="https://github.com/libretro/Genesis-Plus-GX"
COMMIT="c2838c7dc4236fc2fe94e5dbd08b41486067918e"   # 2026-09, master
...
echo "== Genesis Plus GX core build"
...
# ---------------------------------------------------------------- build
# platform=unix gives -fPIC/-shared, the version script that exports retro_* only, and -O2 -DNDEBUG.
# Only the toolchain is swapped for the NDK's, plus the Android link flags (16 KB page alignment for
# Android 15+, -lm/-llog). HAVE_CHD is left at the Makefile default: Mega CD needs it and the core
...
cp -f "$SRC_DIR/genesis_plus_gx_libretro.so" "$UNSTRIPPED"
echo "== Stripping"
